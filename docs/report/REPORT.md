...
2. `cv2.resize(..., INTER_AREA)` of the linear frame to the analysis grid (64 x 48 by
   ...
   per-pixel formulation (634 fps analysis-only on 720p on the 4-vCPU x86 build host).
...
Synthetic suite of 44 clips generated from a YAML config by
...
| metric | value |
|---|---|
| clip-level verdict accuracy | 1.000 |
| hazard-type agreement | 1.000 |
| per-second precision / recall / F1 (`fail` positive) | 1.000 / 1.000 / 1.000 |
| boundary cases correct | 17 / 17 |
| mean temporal IoU vs ground-truth event intervals | 0.899 |
| mean spatial IoU vs ground-truth regions | 0.979 |
...
| metric | value |
|---|---|
| clips with hazards | 25 |
| post-verification pass rate (whole file re-analysed) | 1.000 |
| mean iterations per segment | 1.10 |
| SSIM outside regions | 1.0000 |
